...
function newClients () {
  ...
  return {
    ...
    nameByChannelId: (channelId) => {
      return Object.keys(state).find(name => state[name]?.channel?.id === channelId)
    },
    remove: (name) => {
...
function newCluster () {
  ...
  return {
    ...
    nameByChannelId: (channelId) => {
      return Object.keys(servers).find(name => servers[name]?.channel?.id === channelId)
    },
    remove: (name) => {
      if (servers[name]) {
        delete servers[name]
...
function create () {
  ...
  function onCancelled (notification) {
    cluster.removeLoad(notification.serverName, notification.load)
    const client = clients.get(notification.clientName)
    if (client) {
      client.channel.write(f.notifications.toWire(notification))
  ...
  function handleChannelData (channel, data) {
    switch (data.c) {
      ...
      case NOTIFICATIONS.CANCELLED:
        ...
        onCancelled(f.notifications.fromWire(data))
        break
