...
function newRequests () {
  const state = []
  return {
    add: (id, channelId, procedure, args, affinity, withStatus, withProgress, load = [1]) => {
      state.push({ id, channelId, procedure, args, affinity, withStatus, withProgress, load })
    },
    remove: (id) => {
...
function create () {
  ...
  const executeProcedure = () => {
    ...
    if (!execChannelId) {
      ...
      if (request.withStatus) {
        ...
        reqChannel && reqChannel.write({ c: COMMANDS.RPC_EXECUTE, id: request.id, status: 'no-exec-channel-id' })
      }
      return false
    }
    ...
    if (!execChannel) {
      ...
      if (request.withStatus) {
        const reqChannel = channels.get(request.channelId)
        reqChannel && reqChannel.write({ c: COMMANDS.RPC_EXECUTE, id: request.id, status: 'no-exec-channel' })
      }
      return false
    }
    ...
    execChannel.write({ c: COMMANDS.RPC_EXECUTE, id: request.id, procedure: request.procedure, args: request.args })
    if (request.withStatus) {
      const reqChannel = channels.get(request.channelId)
      reqChannel && reqChannel.write({ c: COMMANDS.RPC_EXECUTE, id: request.id, status: 'dispatched', on: execChannel.id })
    }
    return true
  }
  ...
  function schedule (channel, id, procedure, args, affinity = null, withStatus = false, withProgress = false, load) {
    requests.add(id, channel.id, procedure, args, affinity, withStatus, withProgress, load)
    ...
    if (d.c === COMMANDS.RPC_EXECUTOR) {
      ...
      if (d.result !== undefined || d.error !== undefined || d.progress !== undefined) {
        ...
        if (d.result !== undefined || d.error !== undefined) {
          execContext.removeLoad(request.execChannelId, request.load)
          dispatchedRequests.remove(d.id)
          executeProcedures()
        }
      } else {
        schedule(c, d.id, d.procedure, d.args, d.affinity, d.withStatus, d.withProgress, d.load)
        executeProcedures()
      }
    } else {
