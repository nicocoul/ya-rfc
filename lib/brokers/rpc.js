...
function newChannels () {
  ...
  return {
    set: (c) => {
      logger.info(`set channel ${JSON.stringify(c)}`)
      state[c.id] = c
    },
    get: (id) => {
      return state[id]
...
function newRequests () {
  const state = []
  return {
    add: (id, channelId, procedure, args, affinity, withStatus, withProgress, load = [1], priority, cancelToken) => {
      state.push({ id, channelId, procedure, args, affinity, withStatus, withProgress, load, priority, cancelToken })
    },
    remove: (id) => {
    ...
    getByCancelToken: (cancelToken) => {
      return state.filter(s => s.cancelToken === cancelToken)
    },
    pick: () => {
      return state[0]
    },
...
function create () {
  ...
  function schedule (channel, id, procedure, args, affinity = null, withStatus = false, withProgress = false, load, priority, cancelToken) {
    requests.add(id, channel.id, procedure, args, affinity, withStatus, withProgress, load, priority, cancelToken)
  ...
  function handleData (c, d) {
    if (d.c === COMMANDS.RPC_EXECUTOR) {
      ...
      if (d.result !== undefined || d.error !== undefined || d.progress !== undefined) {
        ...
        events.emit('schedule', d)
        schedule(c, d.id, d.procedure, d.args, d.affinity, d.withStatus, d.withProgress, d.load, d.priority, d.cancelToken)
        executeProcedures()
      }
    } else if (d.c === COMMANDS.RPC_CANCEL) {
      dispatchedRequests.getByCancelToken(d.cancelToken).forEach(request => {
        const execChannel = channels.get(request.execChannelId)
        if (execChannel) {
          execChannel.write({ c: COMMANDS.RPC_CANCEL, id: request.id })
        }
      })
      requests.getByCancelToken(d.cancelToken).forEach(request => {
        requests.remove(request.id)
        const reqChannel = channels.get(request.channelId)
        if (reqChannel) {
          reqChannel.write({ c: COMMANDS.RPC_EXECUTE, id: request.id, result: 'null' })
        }
      })
    } else {
  ...
  function plug (plugin) {
    ...
    plugin.on('new-channel', channel => {
      ...
      events.emit('new-channel', { id: channel.id })
      executeProcedures()
    })
    plugin.on('lost-channel', channel => {
      events.emit('lost-channel', { id: channel.id })
