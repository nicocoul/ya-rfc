...
function newRequests () {
  ...
  return {
    ...
    setDispatched: (id, channelId) => {
      const r = state.find(s => s.id === id)
      if (r) {
        r.dispatched = true
        r.dispatchedOn = channelId
      }
    },
    getAllDispatchedOn: (channelId) => {
      return state.filter(s => s.dispatchedOn === channelId)
    },
    removeByDispatchedOn: (channelId) => {
      state.remove(s => s.dispatchedOn === channelId)
    }
  }
}
...
function create () {
  ...
  const executeProcedure = () => {
    ...
    if (r.withStatus) {
      ...
      reqChannel.write({ c: COMMANDS.RPC_EXECUTE, id: r.id, status: 'started', on: execChannel.id })
    }
    return true
  }
  ...
  function plug (plugin) {
    ...
    plugin.on('lost-channel', channel => {
      requests.getAllDispatchedOn(channel.id).forEach(r => {
        const reqChannel = channels.get(r.channelId)
        if (reqChannel) {
          reqChannel.write({ c: COMMANDS.RPC_EXECUTE, id: r.id, error: `lost server ${channel.id}` })
        }
      })
      requests.removeByDispatchedOn(channel)
