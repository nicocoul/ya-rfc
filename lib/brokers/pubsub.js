...
const { newWrapper, newTransform, newPassTrough, newLocalSubscriptionChannel } = require('../common')
...
function newSubscriptions () {
  ...
  return {
    add: (id, filterSpec, offset = -1) => {
      ...
      const filter = (filterSpec) ? new Function('m', filterSpec) : () => (true)
...
function create (cacheTime) {
  ...
  function handleData (channel, data) {
    if (!data.t || !data.m) return
    if (data.t.includes([TOPICS.RPC_EXECUTOR, TOPICS.RPC_EXECUTE, TOPICS.OS_LOAD])) return
    if (data.t === TOPICS.SUBSCRIBE) {
      _subscribe(channel, data.m.id, data.m.topic, data.m.filter, data.m.offset)
    } else if (data.t === TOPICS.UNSUBSCRIBE) {
      _unsubscribe(channel, data.m.topic)
    } else {
