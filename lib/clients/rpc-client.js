...
function addRpcClientRole (channel, events) {
  ...
  channel.on('data', (data) => {
    ...
    switch (data.c) {
      ...
      case NOTIFICATIONS.FAILED:
        ...
        request.onEnd(new Error(notification.value), undefined)
        break
  ...
  channel.on('close', () => {
    ...
    requests.forEach(s => {
      const error = new Error('lost broker')
      error.code = 'LOST_BROKER'
      s.onEnd(error, undefined)
...
function create (channel) {
  ...
  channel.on('connect', () => {
    ...
    channel.write(f.commands.registerClient.new(rpcClient.name))
  })
  channel.on('close', () => {
