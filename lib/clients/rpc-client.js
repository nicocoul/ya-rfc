...
function addRpcClientRole (channel) {
  ...
  function execute (procedure, args, onResult, options = {}) {
    const { affinity = null, onProgress, onStatus, priority = 0 } = options
    const withProgress = onProgress !== undefined
    const withStatus = onStatus !== undefined
    const request = { id: uuidv4(), procedure, args, affinity, withStatus, withProgress, priority }
  ...
  function isOptions (variable) {
    ...
    if (variable.onProgress || variable.onStatus || variable.priority) return true
    return false
  }
