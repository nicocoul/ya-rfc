...
function newRequests () {
  ...
  return {
    ...
    getByCancelToken: (cancelToken) => {
      return Object.values(state).filter(s => s.request.ct === cancelToken)
    },
    forEach: (fun) => {
    ...
    stats: () => {
      return Object.values(state)
        .map(s => s.request)
        .reduce((accumulated, command) => {
          if (!accumulated[command.pr]) {
            accumulated[command.pr] = 0
          }
          accumulated[command.pr]++
          return accumulated
        }, {})
    }
  }
...
function addRpcClientRole (channel, events) {
  ...
  channel.on('data', (data) => {
    ...
    switch (data.c) {
      ...
      case NOTIFICATIONS.CANCELLED:
        events.emit(EVENTS.CANCELLED, f.events.fromWire(data))
        requests.remove(notification.id)
        request.onEnd(f.errors.fromWire(notification.value), undefined)
        break
  ...
  function execute (procedure, args, onEnd, options = {}) {
    ...
    events.emit(EVENTS.SCHEDULE, f.events.fromWire(command))
    events.emit(EVENTS.EXECUTE, f.events.fromWire(command))
  ...
  function isOptions (variable) {
    ...
    if (variable.onProgress || variable.affinity || variable.priority || variable.cancelToken || variable.load) return true
    return false
  }
  ...
  return { execute, remote, cancel, name, requests }
}
...
function create (channel) {
  ...
  return {
    ...
    events,
    stats: rpcClient.requests.stats
  }
}
