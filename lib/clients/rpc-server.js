...
function newWorker (modulePath) {
  ...
  worker.on('error', (error) => {
    ...
    _executing = false
    emitEvent(NOTIFICATIONS.FAILED, error.message)
  })
  worker.on('close', (data) => {
    logger.warn(`worker close ${data}`)
    _executing = false
    emitEvent(NOTIFICATIONS.FAILED, 'worker was closed')
    events.emit('close', data)
  })
  worker.on('exit', (code) => {
    logger.warn(`worker exit ${code}`)
    _executing = false
    emitEvent(NOTIFICATIONS.FAILED, 'worker excited')
    events.emit('exit', code)
  })
  worker.on('uncaughtException', (err) => {
    logger.error(`uncaughtException ${err.message}`)
    _executing = false
    emitEvent(NOTIFICATIONS.FAILED, `uncaughtException ${err.message}`)
  })
  worker.on('unhandledRejection', (reason) => {
    logger.error('unhandledRejection')
    _executing = false
    emitEvent(NOTIFICATIONS.FAILED, `unhandled rejection ${reason}`)
  })
...
function create (channel, modulePath, options = {}) {
  ...
  function onDoExecute (command) {
    if (!procedures.exists(command.procedure)) {
      const error = `procedure ${command.procedure} not found`
      error.code = 'NO_PROC'
      logger.error(error)
      events.emit(EVENTS.FAILED, f.events.new(command, error.code))
      channel.write(f.notifications.new(NOTIFICATIONS.FAILED, name, command, error.code))
      return
    }
  ...
  function onDoCancel (command) {
    const scheduledRequestIndex = scheduledCommands.findIndex(r => r.id === command.id)
    if (scheduledRequestIndex !== -1) {
      scheduledCommands.splice(scheduledRequestIndex, 1)
      events.emit(EVENTS.CANCELLED, f.events.new(command))
    } else {
      cluster.cancel(command.id)
      events.emit(EVENTS.CANCELLED, f.events.new(command))
    }
    processQueue()
  }
  ...
  channel.on('connect', () => {
    ...
    channel.write(f.commands.registerSerer.new(name, procedures.names(), affinity, workers))
  })
