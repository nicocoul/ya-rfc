...
function newCluster (size, modulePath) {
  ...
  function cancel (id) {
    const workerIndex = workers.findIndex(w => w.command() && w.command().id === id)
    if (workerIndex !== -1) {
      workers[workerIndex].events.removeAllListeners()
  ...
  function isExecuting (command) {
    return workers.find(w => {
      if (w.isExecuting()) {
        const executingCommand = w.command()
        return `${command.procedure} ${JSON.stringify(command.args)}` === `${executingCommand.procedure} ${JSON.stringify(executingCommand.args)}`
      } else {
        return false
      }
    }) !== undefined
...
function create (channel, modulePath, options = {}) {
  ...
  function processQueue () {
    ...
    while (scheduledCommands.length() && (worker = cluster.availableWorker())) {
      ...
      if (cluster.isExecuting(command)) {
        const message = ['cancelled because already executing', 'CANCELLED_ALREADY_EXECUTING']
        channel.write(f.notifications.new(NOTIFICATIONS.CANCELLED, name, command, message))
        events.emit(EVENTS.CANCELLED, f.events.new(command, message))
        processQueue()
      } else {
